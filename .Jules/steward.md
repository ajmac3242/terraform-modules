...
- [2026-06-06] Standardized KMS ARN regex validation to `^arn:aws:kms:[a-z0-9-]+:[0-9]{12}:key/[a-z0-9-]+$` and normalized error messages across 36 modules, remediating regressions introduced by PR #120. Expanded `.github/workflows/ci.yml` matrix to include all 58 repository modules for automated `validate`, `lint`, and `docs` jobs. Verified `aurora_postgresql`, `observability_admin`, `bedrock_agent_core`, `iam`, `lambda`, and `s3` modules pass native `terraform test` and enforced environment hygiene.
