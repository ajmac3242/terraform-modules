...
- [2026-06-11] Remediated regressions introduced by Navigator sync commit `7e99890` (PR #132). Restored comprehensive CI matrix coverage for all 58 modules in `.github/workflows/ci.yml`. Re-standardized KMS ARN regex validation to `^arn:aws:kms:[a-z0-9-]+:[0-9]{12}:key/[a-z0-9-]+$` and standardized validation error messages to explicitly include 'AWS'. Cleaned up 22 modules by removing unused `aws_account_id` and `aws_region` variable declarations. Verified repository health via native `terraform test` in `iam`, `s3`, `aurora_postgresql`, `observability_admin`, `eventbridge`, and `lambda_powertools`. Corrected validation error message typos in `lambda_powertools` and `eventbridge` discovered during code review. Remediated CI failures by removing unused data sources and locals in `ecs_fargate`, `alb_ecs_fargate`, `account_security`, and `step_functions_lambda`. Fixed null template interpolation in `eventbridge` using `coalesce()`. Corrected 'Unsupported argument' errors in `ecs_fargate` and `alb_ecs_fargate` by removing obsolete `aws_account_id` passes to child modules.
