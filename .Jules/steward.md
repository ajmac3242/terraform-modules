...
- [2026-05-04] Reviewed daily PRs (PR #33). Migrated `iam`, `eventbridge_pipes`, and `securityhub` to AWS Provider 6.0. Hardened `securityhub` with manual ARN construction and `securityhub_arn` output (using `.id` to avoid provider 6.0 deprecation). Hardened `eventbridge_pipes` tests with mandatory tag assertions for the nested IAM role submodule. Verified all changes via `terraform test` and cleaned up build artifacts.
