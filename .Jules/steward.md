...
- [2026-06-06] Standardized KMS ARN regex validation to `^arn:aws:kms:[a-z0-9-]+:[0-9]{12}:key/[a-z0-9-]+$` and normalized error messages across 37 modules, remediating regressions introduced by PR #120. Expanded `.github/workflows/ci.yml` matrix to include all 58 repository modules for automated `validate`, `lint`, and `docs` jobs. Resolved CI failures from unused variable declarations and template interpolation errors in `eventbridge`, `ecs_fargate`, and workload modules. Verified `aurora_postgresql`, `observability_admin`, `bedrock_agent_core`, `iam`, `lambda`, `s3`, `step_functions`, and `lambda_powertools` modules pass native `terraform test` and enforced environment hygiene.
