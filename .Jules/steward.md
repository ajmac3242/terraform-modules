...
- [2026-06-07] KMS Regex Standard — Standardized KMS ARN regex validation to `^arn:aws:kms:[a-z0-9-]+:[0-9]{12}:key/[a-z0-9-]+$` across all modules to ensure consistency and prevent malformed inputs.
...
- [2026-06-07] Standardized KMS ARN regex validation library-wide and restored CI workflow coverage for all 58 modules. Cleaned up unused variables. Verified via native `terraform test`.
