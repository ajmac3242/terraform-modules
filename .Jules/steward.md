...
- [2026-06-04] Standardized KMS ARN regex validation across the entire library to ensure consistency and prevent trailing wildcards. Expanded CI workflow matrix to include all 58 modules (48 base, 10 workload) for validation, linting, and docs. Verified `aurora_postgresql`, `observability_admin`, and `bedrock_agent_core` for compliance with organizational standards. All modules pass native `terraform test`.
