...
- [2026-06-08] Remediated regressions introduced by Navigator sync commit f4b67a1 (PR #126). Re-standardized KMS ARN regex validation to `^arn:aws:kms:[a-z0-9-]+:[0-9]{12}:key/[a-z0-9-]+$` across 32 modules and standardized validation error messages to include 'AWS'. Restored comprehensive CI matrix coverage for all 58 modules (48 base, 10 workload) in `.github/workflows/ci.yml`. Verified that `aurora_postgresql` (PITR/IO-Optimized/Extensions) and `observability_admin` (Org rules) hardening remain intact. Confirmed library-wide compliance for `tags` outputs and variable/output descriptions. Verified repository health via native `terraform test` in `aurora_postgresql`, `observability_admin`, `iam`, and `s3`.
